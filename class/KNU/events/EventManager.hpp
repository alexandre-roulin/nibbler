...
namespace KNU
{
	...
	std::shared_ptr<Pool_<T>> EventManager::accommodateEvent()
	{
		...
		return std::static_pointer_cast<Pool_<T>>(eventPools[std::type_index(typeid(T))]);
	}

	template <typename T>
	std::vector<T> EventManager::getEvents()
	{
		return std::static_pointer_cast<Pool_<T>>(eventPools[std::type_index(typeid(T))])->getData();
	}
