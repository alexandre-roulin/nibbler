...
struct LibraryInfo {
	...
	LibraryInfo(std::string const &&ctitle, std::string const &&cpath,
				T ckLibrary) :
			title(ctitle),
			path(cpath),
			kLibrary(ckLibrary) {}
...
void ADynamicLinkLibrary<T, E, SIGNATURE>::setNextKInstance() {
	...
	if (nextLibraryInfo ==
		ADynamicLinkLibrary<T, E, SIGNATURE>::libraryInfo.end()) {
...
template<typename T, typename E, typename SIGNATURE>
LibraryInfo<E> const &
ADynamicLinkLibrary<T, E, SIGNATURE>::getCurrentLibraryInfo() const {
	return *currentLibraryInfo_;
}

template<typename T, typename E, typename SIGNATURE>
LibraryInfo<E> const &
ADynamicLinkLibrary<T, E, SIGNATURE>::getNextLibraryInfo() const {
	return *nextLibraryInfo_;
}

template<typename T, typename E, typename SIGNATURE>
void ADynamicLinkLibrary<T, E, SIGNATURE>::loadDynamicConstructor() {
	...
	if (lib == libraryInfo.end())
		throw (std::range_error(
				"DynamicLinkLibrary:: Library info is not found"));
	...
	if (!(newInstance_ = reinterpret_cast< SIGNATURE >(dlsym(dlHandle_,
															 "newInstance")))) {
		dlError();
		return;
	}
...
template<typename T, typename E, typename SIGNATURE>
std::vector<LibraryInfo<E> > ADynamicLinkLibrary<T, E, SIGNATURE>::libraryInfo(
		0);
...
ADynamicLinkLibrary<T, E, SIGNATURE>::Error::Error(
		std::string const &s) noexcept : std::runtime_error(s), error_(s) {}
...
template<typename T, typename E, typename SIGNATURE>
const char *
ADynamicLinkLibrary<T, E, SIGNATURE>::Error::what() const noexcept { return (error_.c_str()); }